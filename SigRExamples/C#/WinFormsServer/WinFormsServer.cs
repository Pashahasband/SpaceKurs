...
namespace SignalRChat
{
    ...
    public partial class WinFormsServer : Form
    {
        ...
        const string ServerURI = "http://192.168.0.163:8080";
        ...
        private void StartServer()
        {
            ...
            {
                WriteToConsole(string.Format("Error: {0}", e.Message));
                WriteToConsole(string.Format("StackTrace:\n {0}", e.StackTrace));
    ...
    public class MyHub : Hub
    {
        ...
        {
            var testImagePath = "D:\\lena.jpg";
            using(var fileStream = new FileStream(testImagePath, FileMode.Open))
            using(var stream = new MemoryStream())
            {
                fileStream.CopyTo(stream);
                Clients.Caller.getImage(stream);
            }
