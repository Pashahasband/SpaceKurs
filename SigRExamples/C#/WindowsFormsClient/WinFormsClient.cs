...
namespace WinFormsClient
{
    ...
    public partial class WinFormsClient : Form
    {
        ...
        const string ServerURI = "http://192.168.0.163:8080/signalr";
        ...
        private async void ConnectAsync()
        {
            ...
            HubProxy.On<NetworkStream>("GetImage", (stream) =>
                this.Invoke((Action)(() =>
                    {
                        var testFilePath = "D:\\receivedLena.jpg";
                        using(var fileStream = new FileStream(testFilePath, FileMode.CreateNew))
                        {
                            stream.CopyTo(fileStream);
                        }
                        RichTextBoxConsole.AppendText(string.Format("File has been saved on disk D!"));
                    }
                //RichTextBoxConsole.AppendText(string.Format("This method works!!! Got {0}", imageName))
                ))
            );
            try
            {
